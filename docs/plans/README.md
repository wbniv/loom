...
| Date | Plan | Status |
|---|---|---|
| 2026-08-28 | [Skeleton lever — the starvation point is the declared type's arity, not the funnel gate](2026-08-28-skeleton-lever.md) | Diagnosed and pre-registered; **not launched**. Skeleton acceptance (5.49 %) is not anomalous — it sits between both whole-draw controls. The floor's binding conjunct is type-exactness, gated by arrow arity: 26.79 % correct, 63.35 % type-exact given it, and 71.11 % of drafts one arrow short. Sibling failure extended from 12 drafts to 706 (only 2.97 % are one subterm from typechecking). The banked 14B arm already moved the floor RR 19.11 and its 42 floor draws are unscored — D0 costs $0. Power script exits 2: the $4.55 ceiling affords 0.56 power, a powered arm needs ≈ $8.61 |
| 2026-08-28 | [GCP spend, CLI-queryable via BigQuery billing export](2026-08-28-gcp-billing-export.md) | Implemented; `validate` + `shellcheck` + not-yet-exported error path PASS; dataset applied, export enablement gated on the owner's one console click |
| 2026-08-27 | [Runner self-delete: why the GPU never deleted itself](2026-08-27-runner-self-delete.md) | Implemented; PASS — template/instance name drift proven as the refusal cause (IAM, scopes, propagation all ruled out with evidence); 1-line module fix flows to all six roots; offline terraform-test guard fails pre-fix, passes post-fix |
| 2026-08-27 | [Feedback-legibility arm — does a readable narrowing note change what the model writes?](2026-08-27-feedback-legibility-arm.md) | Designed and pre-registered; isolates 8ed72cd's repr fix via a `narrowing_note_render` seam, two concurrent redraft arms x 64 cells. Primary is L1 repair locality (MDE RR 1.20, power 0.93 @ 1.25x); funnel acceptance demoted to descriptive (MDE 1.75 unaffordable). Banked decomp baseline proven pre-fix but kept as anchor only; ceiling $4.55 |
| 2026-08-27 | [Remote-driver survivability: durable log, run manifest, resume path](2026-08-27-driver-survivability-and-resume.md) | Implemented; PASS — root cause proven from the journal (host suspended mid-poll, lost power, no signal reached the EXIT trap). Driver gains a durable local log, a run manifest, `--resume`/`--fetch-only`/`--detach`; 24-check offline regression suite. Cost: the harness's early draft destroyed the diversity bucket (§7); restore pending |
| 2026-08-27 | [Model-scale arm: does elicitation appear at 14B?](2026-08-27-model-scale-arm.md) | Designed and pre-registered; tests the model half of the pilot's row-1 disjunction on Qwen2.5-Coder-14B, the only step that changes parameter count alone. 32B deferred: every A100 quota in the project is 0.0 and 19.85 GB of weights will not fit an L4. Gates E1/E2 carry over, S1 screens 14B against the banked 7B; ceiling $4.50 |
| 2026-08-26 | [Hole elicitation: make the model write a hole before relaxing the gate](2026-08-26-hole-elicitation.md) | Ran. Stage 0 pilot: 4 blocks x 16 cells, all arms SUCCEEDED, no block clears E1 (best 3.47% Wilson-lower vs a 10% bar) and 31 fill draws spliced 0 times. Section 6 row 1 - hole-directed decomposition is not elicitable at 7B; Stage 1 not launched, $4.55 unspent |
