...
**Draws recorded:** 253 in 3030.719 s  
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| gbnf+typemask | full_corpus | 78 | 206 | 39936 | 69 | 1.728 | 6 | 0.0769 | 98.5 | 128 | 10 | 0.8551 | 11.5628 |
| gbnf+typemask | held_out | 24 | 47 | 12288 | 1 | 0.081 | 0 | 0.0 | — | 23 | 1 | 0.0 | 13.327 |

## Funnel outcome by condition × regime

| condition | regime | parse | scope | references | typecheck | accepted |
|---|---|---|---|---|---|---|
| gbnf+typemask | full_corpus | 78 | 2 | 6 | 51 | 69 |
| gbnf+typemask | held_out | 23 | 0 | 0 | 23 | 1 |
...
**Masked draws:** 253  
**Mask steps:** 52224  
**Mask time:** 92.679357 s (0.001774651 s/token; 0.005048389 s/token uncached)  
**Mask share of masked-draw latency:** 0.0308  
**Pruners enabled:** de-bruijn, goal-type, ref-hash  
**Vocabulary:** 152064 tokens  
**Liveness fallbacks:** 9 (steps where the type layer would have emptied a non-empty syntax mask)

### Tokens pruned and time spent, by layer

| layer | tokens pruned | evaluations | seconds |
|---|---|---|---|
| de-bruijn | 533048 | 222043 | 0.19106 |
| goal-type | 84414875 | 301669 | 0.459929 |
| ref-hash | 148430472 | 218814 | 0.255761 |
| syntax | 7685854368 | 52224 | 91.772622 |
...
- **Mask share of masked-draw latency: 0.0308** — the figure to score against.
- Warm: 0.001774651 s/token. Cold: 0.005048389 s/token. Score against both; the caches are part of the design, so the warm number is what a run actually pays and the cold one bounds it.
