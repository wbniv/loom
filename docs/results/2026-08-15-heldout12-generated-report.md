...
**Draws recorded:** 193 in 2558.135 s  
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| gbnf+typemask | held_out | 96 | 193 | 49152 | 7 | 0.142 | 1 | 0.0104 | 415.0 | 97 | 5 | 0.2857 | 13.1648 |

## Funnel outcome by condition × regime

| condition | regime | parse | scope | references | typecheck | accepted |
|---|---|---|---|---|---|---|
| gbnf+typemask | held_out | 94 | 1 | 3 | 88 | 7 |
...
**Masked draws:** 193  
**Mask steps:** 49152  
**Mask time:** 70.239427 s (0.001429025 s/token; 0.004999308 s/token uncached)  
**Mask share of masked-draw latency:** 0.0276  
**Pruners enabled:** de-bruijn, goal-type, ref-hash  
**Vocabulary:** 152064 tokens  
**Liveness fallbacks:** 6 (steps where the type layer would have emptied a non-empty syntax mask)

### Tokens pruned and time spent, by layer

| layer | tokens pruned | evaluations | seconds |
|---|---|---|---|
| de-bruijn | 510095 | 166061 | 0.141925 |
| goal-type | 85700224 | 231328 | 0.331024 |
| ref-hash | 141074069 | 163677 | 0.195011 |
| syntax | 7226140263 | 49152 | 69.571463 |
...
- **Mask share of masked-draw latency: 0.0276** — the figure to score against.
- Warm: 0.001429025 s/token. Cold: 0.004999308 s/token. Score against both; the caches are part of the design, so the warm number is what a run actually pays and the cold one bounds it.

## Outstanding by rule, not by omission

- R3's hand-scored rubric on held-out successes is outstanding for 1 draws that met the mechanical floor. The metric is partly human and this line is what keeps it from being silently dropped.
