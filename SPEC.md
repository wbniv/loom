...
1. **Prior starvation — bootstrap begun.** The mask guarantees
   ...
   residue for this spec rather than corpus problems; all three are now closed.
   Two by [the polymorphism and Bool-elimination plan](docs/plans/2026-08-13-polymorphism-and-bool-elimination.md):
   a *definition* is polymorphic after all — §2.3.1 checks its term at its type's
   `forall` depth (§3.1.3), which costs no tag, though **instantiating** a
   polymorphic reference still has no rule, so a generic definition is called
   only through a monomorphic one — and `Bool` gained an elimination form, `if`
   (§2.1 tag 12, §3.1.4), the one tag either decision spends. The third is
   closed by §4.3's kind 7 and §5.1.3: `extern` objects encode the arithmetic
   the corpus assumes, and the five assumed-base externs are pinned there.
   Fluency remains unmeasured — a seed set is not a result either.
